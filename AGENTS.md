...
- **Hash** (default): `xxh3_64(canonical_bytes(key), seed=0) % num_dbs` — supports int, string, and bytes keys uniformly. All writers use the same Python-based hash via `routing.xxh3_db_id()`. `num_dbs` is either provided explicitly or computed from `max_keys_per_shard` (= `ceil(count / max_keys_per_shard)`). Hot paths (per-partition / per-batch) build a single resolved closure via `routing.make_hash_router(num_dbs, algorithm, *, key_type=None)`; passing `key_type=Literal["int"|"str"|"bytes"]` selects a type-specialised digest path that skips per-call `isinstance` dispatch in `canonical_bytes`. Spark/Dask/Ray writers infer `key_type` once from the key column dtype; reader and Python writer use the generic 3-branch fallback.
- **CEL**: User-provided CEL expression evaluated at write time (shard assignment) and read time (routing). The expression must produce **consecutive 0-based integer shard IDs** (e.g., `shard_hash(key) % 100u` produces IDs 0–99). A custom `shard_hash()` function is available in CEL expressions (wraps `xxh3_64`). `num_dbs` is always discovered from data (`max(db_id) + 1`); it must not be provided explicitly. Optional `boundaries` field enables `bisect_right`-based routing for range-like patterns. Requires `cel` extra (`cel-expr-python`). `ShardingSpec` fields: `cel_expr`, `cel_columns`, `boundaries` (optional). Per-batch and per-partition CEL closures call `cel.compile_cel_cached(expr, columns_key)` (process-local `lru_cache(maxsize=64)`); `SnapshotRouter` eager-compiles the CEL expression in `__init__` and reuses the compiled instance across `route_one` and `route_with_context`. The `_compile_cel_cached` private alias remains for back-compat.
