# Test Coverage Gaps: What Was Added

73 new tests across 12 files, targeting states and error paths that were previously uncovered. Organised by severity.
...
- Calling `route_key()` *without* `routing_context` on a multi-column CEL manifest — raises `ValueError` (the router cannot function without the extra columns)
...
- Manifest with missing `db_id`s (e.g. only shards 0 and 2 out of 3) — `SnapshotRouter` pads the gap with a synthetic null shard; `shard_details()` shows `row_count=0` for shard 1
...
### `test_prometheus_completeness.py` (15 tests)  `test_otel_completeness.py` (6 tests)
...
- **Silently-ignored events** (by design, not mapped to instruments): `SHARDING_COMPLETED`, `MANIFEST_PUBLISHED`, `CURRENT_PUBLISHED`, `READER_INITIALIZED`, `READER_REFRESHED`, `READER_CLOSED`, `SHARD_WRITE_RETRIED`, `SHARD_WRITE_RETRY_EXHAUSTED` — verified they do not raise
