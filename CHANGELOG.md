...
- **Default S3 run-registry credential resolution now uses only writer-level S3 settings** (`WriteConfig.credential_provider` / `VectorWriteConfig.credential_provider` and `s3_connection_options`). Manifest-level S3 settings (`ManifestOptions.credential_provider` / `ManifestOptions.s3_connection_options`) remain scoped to the default S3 manifest store and are no longer consulted for run-registry S3 access. **Migration**: if you previously configured S3 credentials only via `ManifestOptions`, copy `credential_provider` and `s3_connection_options` to the top-level `WriteConfig` / `VectorWriteConfig` as well, or pass an explicit `run_registry=` instance. Otherwise the run registry will be silently disabled (the writer logs a failure and continues without a run record).
