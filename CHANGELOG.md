...
- **Default S3 run-registry credential resolution now uses only writer-level S3 settings** (`WriteConfig.credential_provider` / `VectorWriteConfig.credential_provider` and `s3_connection_options`). Manifest-level S3 settings remain scoped to the default S3 manifest store and are not used for run-registry S3 access.
