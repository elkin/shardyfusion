...
from shardyfusion.writer.dask.single_db_writer import DaskCacheContext
...
def test_bytes_rate_limiter_acquire_called_with_byte_counts(
    ...
    # 7 rows / batch_size 3 → 2 full batches (3 rows) + 1 trailing partial (1 row)
    ...
    assert bucket.acquire_calls == [33, 33, 11]
...
def test_single_db_retry_uses_new_attempt_id(
    ...
    monkeypatch.setattr(
        "shardyfusion.writer.dask.single_db_writer.cleanup_losers",
...
def test_single_db_retry_records_succeeded_run_record(
    ...
    ddf = _make_dask_df([{"id": k} for k in range(5)], npartitions=1)
    result = write_single_db(
        ddf,
        config,
        key_col="id",
