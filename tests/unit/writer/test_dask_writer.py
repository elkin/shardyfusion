"""Tests for the Dask DataFrame sharded writer."""
...
# Force synchronous scheduler for deterministic, single-threaded tests.
...
def _make_config(
    num_dbs: int = 4,
    *,
    factory: _TrackingFactory | DbAdapterFactory | None = None,
...
def _make_dask_df(records: list[int], key_col: str = "id") -> dd.DataFrame:
    """Create a simple Dask DataFrame from integer keys."""
    pdf = pd.DataFrame({key_col: records, "value": [f"v{r}" for r in records]})
    return dd.from_pandas(pdf, npartitions=max(1, len(records) // 10 or 1))
...
def test_hash_routing_round_trip() -> None:
    ...
    ddf = _make_dask_df(list(range(40)))

    result = write_sharded_dask(
        ddf,
        config,
        key_col="id",
        value_spec=ValueSpec.json_cols(["id", "value"]),
    )
...
def test_range_explicit_boundaries() -> None:
    ...
    ddf = _make_dask_df(list(range(30)))
...
def test_range_without_boundaries_raises() -> None:
    ...
    with pytest.raises(ConfigValidationError, match="boundaries"):
        write_sharded_dask(
            ...
            config,
            key_col="id",
            value_spec=ValueSpec.json_cols(),
        )
...
def test_batch_flushing() -> None:
    ...
    ddf = _make_dask_df(list(range(7)))
    ...
    # With batch_size=3 and 7 records: 2 full batches + 1 final
...
def test_min_max_key_tracking() -> None:
    ...
    ddf = _make_dask_df([10, 20, 30, 60, 70, 80])
...
def test_binary_col_value_spec() -> None:
    factory = _TrackingFactory()
    config = _make_config(num_dbs=1, factory=factory)

    pdf = pd.DataFrame({"id": [1, 2, 3], "payload": [b"a", b"b", b"c"]})
...
def test_data_integrity(tmp_path: pathlib.Path) -> None:
    ...
    # Verify every record was written
    key_encoder = make_key_encoder(config.key_encoding)
    for r in records:
        assert key_encoder(r) in all_kv
...
def test_rate_limited_write() -> None:
    ...
    ddf = _make_dask_df(list(range(5)))
...
def test_multiple_partitions() -> None:
    """Verify correct behavior when the Dask DF has many partitions."""
    factory = _TrackingFactory()
    config = _make_config(num_dbs=4, factory=factory)

    pdf = pd.DataFrame({"id": list(range(40)), "value": [f"v{i}" for i in range(40)]})
...
def test_single_shard() -> None:
    factory = _TrackingFactory()
    config = _make_config(num_dbs=1, factory=factory)
    ddf = _make_dask_df(list(range(20)))
...
def test_uneven_distribution() -> None:
    factory = _TrackingFactory()
    config = _make_config(num_dbs=8, factory=factory)
    ddf = _make_dask_df(list(range(5)))
...
def test_u32be_key_encoding() -> None:
    factory = _TrackingFactory()
    config = _make_config(
        num_dbs=4,
        factory=factory,
