...
class TestVectorShardingFindability:
    def test_cluster_findability_and_manifest_roundtrip(
        ...
        reader = ShardedVectorReader(
            ...
            manifest_store=_make_manifest_store(prefix, cred_provider, s3_conn_opts),
        )

        # Manifest round-trip: reader's centroids equal the writer's exactly.
        ...
        for rec in records[::8]:
            ...
            # Querying the vector by itself (no shard_ids → automatic routing)
            # finds it, in exactly its own shard.
