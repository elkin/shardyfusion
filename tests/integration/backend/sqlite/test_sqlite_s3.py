...
Includes round-trip tests for the plain :class:`SqliteReaderFactory` /
:class:`AsyncSqliteReaderFactory`, plus adaptive-factory tests that exercise
:class:`AdaptiveSqliteReaderFactory` /
:class:`AsyncAdaptiveSqliteReaderFactory` end-to-end against moto S3.
"""
...
def _write_three_shards(tmp_path: Path, run_id: str) -> tuple[list[str], list[int]]:
    """Write 3 small SQLite shards to S3, return (db_urls, db_byte_sizes)."""
    db_urls: list[str] = []
    db_bytes_per_shard: list[int] = []
    for i in range(3):
        db_url = f"{_PREFIX}/shards/run_id={run_id}/db={i:05d}/attempt=00"
...
class TestAdaptiveSqliteReaderFactoryS3:
    ...
    def test_default_thresholds_select_download_for_small_shards(
        ...
        db_urls, sizes = _write_three_shards(tmp_path, run_id="adaptive-A")
    ...
    def test_low_per_shard_threshold_selects_range_mode(
        ...
        db_urls, sizes = _write_three_shards(tmp_path, run_id="adaptive-B")
    ...
    def test_new_run_id_replaces_cached_sub_factory(
        ...
        db_urls_a, sizes_a = _write_three_shards(tmp_path, run_id="adaptive-C1")
        manifest_a = _StubManifest(run_id="adaptive-C1", shard_sizes=sizes_a)
        db_urls_b, sizes_b = _write_three_shards(tmp_path, run_id="adaptive-C2")
...
class TestAsyncAdaptiveSqliteReaderFactoryS3:
    ...
    async def test_default_thresholds_select_download(
        ...
        db_urls, sizes = _write_three_shards(tmp_path, run_id="async-adaptive-A")
    ...
    async def test_low_threshold_selects_range_mode(
        ...
        _db_urls, sizes = _write_three_shards(tmp_path, run_id="async-adaptive-B")
