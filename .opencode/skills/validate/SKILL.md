...
2. **CLAUDE.md** — Reflects the current module structure, commands, architecture, testing notes, and error hierarchy.
3. **AGENTS.md** — Reflects the current project structure, build commands, test commands, and coding style.
4. **docs/ directory** — MkDocs source files are consistent with code changes. **Run the `validate-docs` skill** for automated checks of API references, config fields, extras, and CLI surface.
